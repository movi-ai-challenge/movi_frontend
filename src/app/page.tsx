    ...
    <AppScreen
      ...
      footer={
        <>
          ...
          <p className="text-center text-xs text-[var(--color-text-muted)]">MOVI · 음성 중심 포용 금융</p>
        </>
      }
    >
      <span
...
// 목업 빠른 메뉴에는 "보호자" 항목이 있으나, 확정된 보호자 정책
// (docs/GUARDIAN_ALERT_POLICY.md)이 보호자용 금융 대시보드를 금지하므로
// 진입점을 두지 않는다.
const QUICK_MENU = [
...
function SignedInHome({ displayName }: { displayName: string }) {
  ...
  const mainAccount = accounts.find((account) => account.id === defaultAccountId) ?? accounts[0] ?? null;
    ...
    <AppScreen className="gap-5 pb-8 pt-6">
      ...
      <header className="flex items-start justify-between gap-3">
        ...
        <nav aria-label="바로가기" className="flex gap-2">
          <IconLink href="/notifications" label="알림" symbol="🔔" />
      ...
      {loadError ? (
        ...
        <SurfaceCard accent className="p-5">
          {mainAccount ? (
            ...
            <div className="flex flex-col gap-3">
              <p className="text-[15px] text-[var(--color-text-muted)]">아직 연결된 계좌가 없어요.</p>
      ...
      {/* 음성 명령 */}
      <section aria-labelledby="voice-heading" className="flex flex-1 flex-col items-center justify-center gap-4">
