 ...
 *
 * 인증 보호 대상이 아니다. 저시력 사용자가 로그인하기 전에
 * 고대비와 큰 글씨를 켤 수 있어야 하기 때문이다.
 * 계좌 항목은 세션이 있을 때만 보여준다.
 */
    ...
    <AppScreen className="gap-6 pb-10 pt-6">
      ...
      {session ? (
        <section aria-labelledby="account-settings-title">
          ...
          {loadError ? (
            <p role="alert" className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-4 text-[15px] text-[var(--color-danger)]">
              {loadError}
            </p>
          ) : (
