...
| P1-1 | 직접 입력 화면의 남은 Mock 제거 | 완료 | 고정 수취인·가짜 음성 입력·판정과 미사용 서비스·store·타입 제거 완료 | 프런트 |
...
정리 전 `/transfer`의 수취인 목록은 `recipientService.ts`의 고정 Mock이었고, 화면 안의 “음성으로 입력”도 실제 녹음이 아닌 타이머 기반 시연이었다. 실제 송금은 실행하지 않도록 차단했지만 실제 기능으로 오해할 수 있는 표현이 남아 있었다.

2026-08-27 반영: `/transfer`와 `/transfer/review`의 실제 사용자 경로에서 고정 수취인, 타이머 기반 음성 입력과 가짜 확인·취소 판정 의존성을 제거했다. 직접 입력은 서버 검증이 아닌 검토용 정보임을 표시하며, 실제 이체·FDS·보호자 결과를 만들지 않는다. 삭제 승인 후 미사용 `recipientService`, `guardianRiskAlertService`, `VoiceTransferDecision`, 이전 transfer result store 상태와 관련 Mock 타입·데이터도 제거했다. 폐기된 공개 URL은 금융 결과를 만들지 않고 `/accounts`로 안전하게 복귀시키기 위해 리다이렉트 라우트만 유지한다.

- 실제 녹음처럼 보이는 타이머 기반 입력을 제거한다.
- 직접 입력 화면은 계약 확정 전까지 “정보 입력·검토만 가능, 이체 미실행”을 일관되게 표시한다.
...
- 프런트 `/transfer`는 Mock 수취인을 읽고 검토까지만 수행한다.
