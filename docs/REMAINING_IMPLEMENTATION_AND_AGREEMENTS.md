...
| P1-1 | 직접 입력 화면의 남은 Mock 제거 | 프런트 | 가짜 음성 입력과 사용되지 않는 Mock 서비스·타입을 실제 경로에서 정리 | 프런트 |
...
현재 `/transfer`의 수취인 목록은 `recipientService.ts`의 고정 Mock이고, 화면 안의 “음성으로 입력”도 실제 녹음이 아닌 타이머 기반 시연이다. 실제 송금은 실행하지 않도록 이미 차단했지만 실제 기능으로 오해할 수 있는 표현이 남아 있다.
...
- 공통 API의 동시 401 요청이 refresh 한 번으로 합쳐지는지 검증 완료.
- 로그아웃·refresh 실패 시 인증, 금융, 송금 복구 키가 모두 지워지는지 검증 완료.
