...
- PIN 최초 등록은 가입 수단과 무관하게 인증된 사용자면 누구나 수행한다. 카카오 신규
  가입자는 가입 마지막 단계에서, 일반 가입자는 가입 직후 또는 설정 화면에서 등록한다.
  서버는 비밀번호만 있는 자격증명 행에 PIN을 채워 넣고, `pin_hash`가 이미 차 있을
  때만 `AUTH_4090`으로 거절한다.
- 카카오 신규 가입자에게만 PIN 등록이 필수다. 그 사용자에게는 아이디·비밀번호가 없어
  PIN이 유일한 추가 로그인 수단이다. 일반 가입자는 건너뛸 수 있다.
