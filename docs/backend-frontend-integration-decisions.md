...
> 확정 기준: 2026-08-25 역할별 구현 점검
> 실행 우선순위와 현재 상태: [IMPLEMENTATION_SOURCE_OF_TRUTH.md](IMPLEMENTATION_SOURCE_OF_TRUTH.md)

## 1. 문서 우선순위

이 문서는 현재 확정된 제품 정책과 API 통합 계약을 기록한다. 회의 준비 문서, 과거 감사 결과, Mock 화면과 충돌하면 이 문서를 우선한다. 실제 최신 백엔드 Controller·DTO가 다르면 `확인 필요`로 기록하고 담당 팀과 계약을 다시 확정한다.
...
- 접근성 설정의 새로고침 후 유지 위치는 `확인 필요`다. 계정 저장이 필요하면 백엔드 API를 먼저 합의한다.
...
- 프런트의 직접 AI/FDS 호출
- Google TTS 사용은 `확인 필요`; 현재 MVP 안내는 브라우저 기기 TTS 기준
...
4. 음성 누락 slot 재질문과 확인·취소
5. LOW 완료, MEDIUM 완료+알림, HIGH 차단+알림
