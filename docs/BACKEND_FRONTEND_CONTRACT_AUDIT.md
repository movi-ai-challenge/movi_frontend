...
| PR | 판단 | 병합 전 조치 |
|---|---|---|
| #1 카카오 로그인 | 수정 필수 | URL query의 Access/Refresh 토큰 파싱을 제거한다. 실제 백엔드 콜백은 JSON 응답이며 토큰 전달·보관 방식과 CORS/리다이렉트 계약을 먼저 확정한다. |
...
1. 카카오 콜백은 JSON 토큰 응답이다. 프런트 앱으로 안전하게 돌아오는 리다이렉트 또는 BFF/쿠키 방식이 확정되지 않았다.
2. `SecurityConfig`는 `/api/openbanking/callback`을 공개 경로로 두지 않지만 Controller 주석은 인증되지 않은 콜백이며 `state`가 신원 증명이라고 설명한다.
3. 별도 CORS 설정이 실제 코드에서 확인되지 않았다. 프런트와 백엔드를 다른 origin으로 실행할 경우 허용 origin과 credentials 정책이 필요하다.
